...
**R-009 审查结论**: ⬜ 待 Worker 修复 — P0 trail brake duration 估算问题和 P1 状态机回退/指针风险需修复后方可合并
...
#### 遗留项（P0/P1 已闭环，以下 P2 需 Worker 修复后方可合并）

| 编号 | 级别 | 问题 | 修复要求 | 修改文件 |
|:---:|:---:|------|------|------|
| L-11 | P2 | `segment_id` 仍为 `const char*`，存在悬空指针风险 | 改为 `char segment_id[32] = {0}`，同步更新 CBrakeEvent struct（c_api.h）、C API 拷贝逻辑（c_api.cpp）、FFI 绑定（engine_ffi.dart 中 CBrakeEvent 的 segment_id 改为 `Array<Uint8>` 32 字节 + Dart 端 Utf8 解码） | brake_detector.h, c_api.h, c_api.cpp, engine_ffi.dart |
| L-12 | P2 | 缺少 RELEASING→BRAKING 回退和 trail brake per-point 的专门单元测试 | 在 test_main.cpp 新增 Test 9/10：(1) 测试 RELEASING 阶段 lg 再次低于 kBrakeOnThreshold 时回退到 BRAKING，最终产生 1 个合并事件；(2) 测试 trail_brake_duration_ms 在完整释放流程中计算正确（80%→20% 时间差） | test_main.cpp |

**R-009 闭环结论**: ⬜ P0/P1 ✅ 已闭环，P2 遗留项升级为合并阻塞 — Worker 修复 L-11 + L-12 后 Monitor 再验证，方可合并
